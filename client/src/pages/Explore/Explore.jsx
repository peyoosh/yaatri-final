...
export default function Explore() {
  ...
  return (
    <div className="flex flex-col bg-slate-50" style={{ minHeight: 'calc(100vh - 80px)', marginTop: '80px' }}>
      ...
      {/* Header */}
      <header className="bg-white h-20 border-b border-slate-100 px-8 flex items-center justify-between shadow-sm shrink-0">
        <div className="flex items-center gap-5">
          <button
            onClick={() => navigate(-1)}
            className="p-2.5 rounded-xl hover:bg-slate-100 text-gray-500 hover:text-brand-slate transition-colors cursor-pointer"
          >
            <ArrowLeft className="w-6 h-6" />
          </button>
          <div>
            <span className="text-xs font-bold text-brand-pink uppercase tracking-widest block">YAATRI HUB</span>
            <h1 className="text-xl font-extrabold text-brand-slate tracking-tight flex items-center gap-2 mt-0.5">
        ...
        <div className="max-w-4xl w-full mx-auto flex flex-col gap-8">
          {messages.map(m => (
            <motion.div
              ...
              className={`flex flex-col max-w-[80%] ${m.type === 'user' ? 'self-end items-end' : 'self-start items-start'}`}
            >
              {/* Bubble */}
              <div className={`p-5 text-base leading-relaxed ${
                m.type === 'user'
                  ? 'bg-brand-blue text-white rounded-3xl rounded-tr-sm shadow-md'
                  : 'bg-white text-slate-800 rounded-3xl rounded-tl-sm border border-slate-100 shadow-sm font-medium'
              }`}>
                {m.text}
              </div>
              ...
              {m.redirectTo && ROUTE_LABELS[m.redirectTo] && (
                <button
                  ...
                  className="mt-3 flex items-center gap-2 text-sm bg-brand-green/10 hover:bg-brand-green/20 border border-brand-green/30 text-brand-green rounded-xl px-4 py-2 transition-all cursor-pointer font-semibold"
                >
          ...
          {/* Thinking indicator */}
          {isLoading && (
            <div className="self-start flex items-center gap-3 p-5 bg-white/70 border border-slate-100 rounded-3xl rounded-tl-sm shadow-sm">
              <Loader2 className="w-5 h-5 text-brand-blue animate-spin" />
              <span className="text-sm font-semibold text-gray-400">Consulting Himalayan registers…</span>
            </div>
          )}
      ...
      {/* Footer input */}
      <footer className="bg-white border-t border-slate-100 p-8 shrink-0">
        <div className="max-w-4xl w-full mx-auto flex flex-col gap-5">

          {/* Quick prompts (only on first message) */}
          {messages.length <= 1 && !isLoading && (
            <div className="flex gap-3 overflow-x-auto pb-1 no-scrollbar text-sm font-bold text-gray-600">
              {prompts.map(p => (
                <button
                  key={p}
                  onClick={() => setUserInput(p.slice(3).trim())}
                  className="px-5 py-2.5 rounded-full border border-slate-200 hover:border-brand-blue bg-slate-50 hover:bg-brand-blue/5 transition-all shrink-0 cursor-pointer whitespace-nowrap"
                >
                  {p}
                </button>
              ))}
          ...
          <form
            ...
            className="flex items-center gap-4 bg-slate-50 border border-slate-200 rounded-full px-6 py-4 focus-within:border-brand-blue focus-within:ring-2 focus-within:ring-brand-blue/10 transition-all"
          >
            <Compass className="w-6 h-6 text-gray-400 shrink-0" />
            <input
              ref={inputRef}
              type="text"
              ...
              disabled={isLoading}
              className="flex-1 bg-transparent border-none text-base text-brand-slate font-semibold focus:outline-none placeholder-gray-400"
            />
            <button
              type="submit"
              disabled={isLoading || !userInput.trim()}
              className="p-3 rounded-full bg-brand-blue hover:bg-brand-blue/90 text-white disabled:bg-gray-200 disabled:text-gray-400 transition-colors cursor-pointer shrink-0"
            >
